...
class Binomial(Distribution):
    ...
    def replace_stats_with_data(self):
        ...
        self.n = len(self.data) # n is size of the data
        p = 0
        for d in self.data:
            if d > 0:
                p += 1
    ...
    def pdf(self, k):
        """Probability density function calculator for the binonial distribution.
        ...
        combination = math.factorial(self.n) / (math.factorial(self.n - k) * math.factorial(k))
        pdf_val = combination * self.p**k * (1 - self.p)**(self.n -k)
        return pdf_val
